...
interface AlertItemProps { severity: AlertSeverity; name: string; chipLabel: string; desc: string; score: number; elapsed: string; action: string; onClick?: () => void; }
...
export function AlertItem({ severity, name, chipLabel, desc, score, elapsed, action, onClick }: AlertItemProps) {
  const s = S[severity];
  return (
    <div
      ...
      className={cn(
        "flex gap-3 p-3 rounded-xl mb-1.5 border border-transparent transition-transform",
        onClick && "cursor-pointer hover:translate-x-0.5 focus:outline-none focus:ring-2 focus:ring-brand-500/40",
        s.bg
      )}
    >
      ...
      <button type="button" className="self-center text-xs font-bold text-warm-600 bg-white border border-warm-200 rounded-lg px-2.5 py-1.5 flex-none hover:border-warm-300 hover:text-warm-800 transition-colors whitespace-nowrap">{action}</button>
    </div>
  );
}
